...
const createNew = async (userId, data, files = []) => {
  ...
  if (files && files.length > 0) {
    const uploadResults = await Promise.all(
      files.map(file => CloudinaryProvider.streamUpload(file.buffer, 'review-attachments'))
    )
...
const getReviewsByProductId = async (productId, page = 1, limit = 10) => {
  try {
