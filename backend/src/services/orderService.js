...
const createNew = async ({ userId, reqBody }) => {
  ...
  for (const item of reqBody.items) {
    ...
    if (product.stockQuantity < item.quantity) {
      throw new ApiError(StatusCodes.BAD_REQUEST, `Sản phẩm ${product.name} không đủ hàng!`)
    }
...
const updateOrderStatus = async (orderId, status) => {
  ...
  // Khi đơn hàng được xác nhận, trừ tồn kho
  if (status === STATUS_ORDER.CONFIRMED && order.items && order.items.length > 0) {
    const warehouseProducts = order.items.map(item => ({
      productId: item.productId,
      quantity: item.quantity
    }))
    ...
    try {
      await warehouseService.exportProducts(
        null,
        warehouseProducts,
        `Xuất hàng cho đơn hàng #${orderId}`,
        null,
        orderId
      )
    } catch (warehouseError) {
      console.error('Lỗi khi trừ tồn kho:', warehouseError)
    }
  }
