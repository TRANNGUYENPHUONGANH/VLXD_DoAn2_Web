  ...
  const result = await GET_DB().collection(CART_COLLECTION_NAME).aggregate([
    ...
    { $unwind: { path: '$productDetail', preserveNullAndEmptyArrays: true } },
    {
      $addFields: {
        ...
        'items.productImage': '$productDetail.images',
        'items.slug': '$productDetail.slug',
        'items.isActive': '$productDetail.status',
        'items.price': '$productDetail.price',
        'items.unit': '$productDetail.unit',
        'items.stockQuantity': '$productDetail.stockQuantity'
      }
    },
    {
      $group: {
          ...
          $push: {
            $cond: [
              ...
              '$$REMOVE',
              {
                ...
                image: { $arrayElemAt: ['$items.productImage', 0] },
                slug: '$items.slug',
                price: '$items.price',
                unit: '$items.unit',
